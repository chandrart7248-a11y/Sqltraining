...
select date(payment_date) as payment_day, sum(amount) as total_amount from payment group by date(payment_date) having count(payment_id)>100;
...
select c.customer_id, c.first_name, c.last_name, count(r.rental_id) as total_rentals from customer c
left join rental r
